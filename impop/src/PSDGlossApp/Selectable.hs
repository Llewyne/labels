...
--    drawHover :: RealFrac r => PlanarSubdivision s v e f r -> PSE s -> Picture
    ...
    drawHover psd (D i) = hoverDrawEdge (_arc i) (psd ^. dataOf i, psd ^. dataOf (twin i)) $ _core $ edgeSegment i psd
...
--    drawCurve psd (D i) = Color red $ glossify $ Bezier $ map ((psd ^.) . locationOf) $ curveVerts psd i
    ...
    drawCurveSegment psd i = curveDrawEdge (_arc i) (psd ^. dataOf i, psd ^. dataOf (twin i)) $ _core $ edgeSegment i psd


curveDrawEdge :: EdgeDrawer s v e r
curveDrawEdge i e s = Color red $ glossify $ dilate s 2



selecDrawVert :: VertDrawer s v r
selecDrawVert i v p = Color orange $ uncurry Translate (glossify p) $ circleSolid 17

selecDrawEdge :: EdgeDrawer s v e r
selecDrawEdge i e s = Color orange $ glossify $ dilate s 7

selecDrawFace :: FaceDrawer s v f r
...
--  }
  where
    ...
    drawSelection psd (D i) = selecDrawEdge (_arc i) (psd ^. dataOf i, psd ^. dataOf (twin i)) $ _core $ edgeSegment i psd
