...
placeLabelsSATExtensible ul f = do
    ...
    let clauses = setAssignmentClauses ml ++ setOverlapClauses ml
    -- putStrLn $ show $ head ml 
...
printClauses ul simplified= do
    let ml = mapVars (map (makeUnplacedLabelExt (simplify simplified)) ul)
    let allClause = (map (makeHardClause maxWeight) (setAssignmentClauses ml)) ++ (map (makeHardClause (maxWeight-1)) (setOverlapClauses ml)) ++ map setSoftClauses (concat ml)
...
makeHardClause :: Float -> [Int] -> Clause
makeHardClause w c = (c,w)
...
makeLabelExt ml i = Label clue port len `debug` (show port ++ "|" ++ show len ++ "|" ++ show i)
        where
            clue = snd $ fst $ snd $ ml!!(i-1)
...
makeUnplacedLabelExt ls ul = ([(p,ls)|p<-fst ul,ls<-(filter (\x-> x >= minLength p) [(m*stepsOf)|m<-[0..eMax]])],snd ul)
    where
...
setAssignmentClauses :: [[(Int,(UnplacedLabelExt,Int))]] -> [[Int]]
setAssignmentClauses ml = concatMap ( map (\((a,_),(b,_))->[-a,-b])) (map pairs ml) -- make sure 1 of 2 ports is selected for every line

setOverlapClauses :: [[(Int,(UnplacedLabelExt,Int))]] -> [[Int]]
setOverlapClauses ml = map (\((a,_),(b,_))->[-a,-b]) (filter overlap (filter differentPort (pairs (concat ml))))

setSoftClauses :: (Int,(UnplacedLabelExt,Int)) -> Clause
setSoftClauses (c,(ul,p)) = ([c],(eMax*stepsOf) - (((snd ((fst ul)!!(p-1)))))) --`debug` (show c ++ show ((fst ul)!!(p-1)))
